document.addEventListener("DOMContentLoaded", function () {
  ...
  function gameLoop() { 
    intervalId = setInterval(() => {
        if(isGameOver()){
            clearInterval(intervalId);
            alert("Game Over, Refresh the page for new Game" + '\n' + 'Your Score:' + score);
            gameStarted = false;
            return;
        }
      updateSnake();
      drawFoodAndSnake();
      drawScoreBoard();
    }, gameSpeed);     // Update snake and redraw every 100 ms (10 times per second
  }
  function runGame() {
    if (!gameStarted) {
      gameStarted = true;
